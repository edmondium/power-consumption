...
def make_input_fn(data_file,
  ...
  def _filter_data(data, labels):
    ...
    rand_num = tf.random.uniform([], 0, 1, dtype=tf.float64)
  ...
  '''dataset = tf.contrib.data.CsvDataset([data_file, ],
                                       ...
                                       header=True,
                                       select_cols=cols)'''
  dataset = tf.data.experimental.CsvDataset([data_file, ],
                                       record_defaults,
                                       header=True,
...
def model_fn(features, labels, mode, params):
  ...
  seq_data = tf.compat.v1.feature_column.input_layer(features, feat_cols)
  if not params['use_keras']:
    #tf.logging.info('Tensorflow authoring')
    tf.compat.v1.logging.info('Tensorflow authoring')
    ...
    cells = [tf.compat.v1.nn.rnn_cell.DropoutWrapper(
      tf.nn.rnn_cell.LSTMCell(params['lstm_size']), input_keep_prob=1 - params['dropout_rate']) for _ in range(params['num_layers'])]
    ...
    flatten_outputs = tf.compat.v1.layers.Flatten()(outputs)
    ...
    logits = tf.compat.v1.layers.Dense(params['num_appliances'])(flatten_outputs)
  else:
  ...
  loss = tf.compat.v1.losses.sigmoid_cross_entropy(multi_class_labels=labels, logits=logits)
  ...
  precision = tf.compat.v1.metrics.precision(labels=labels, predictions=predicted_classes)
  #recall = tf.metrics.recall(labels=labels, predictions=predicted_classes)
  recall = tf.compat.v1.metrics.recall(labels=labels, predictions=predicted_classes)
  ...
  train_op = optimizer.minimize(loss_avg, global_step=tf.compat.v1.train.get_global_step())
