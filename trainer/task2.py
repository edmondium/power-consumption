...
def test(hparams, estimator):
  ...
  """
  test_input = lambda: LFNO.make_input_fn(
  ...
  tf.compat.v1.logging.info('test_data.shape={}'.format(test_data.shape))
...
def run_experiment(hparams):
  ...
  train_input = lambda: LFNO.make_input_fn(
  ...
  eval_input = lambda: LFNO.make_input_fn(
  ...
  tf.compat.v1.logging.info('model dir {}'.format(model_dir)) # TensorFlow 2
  ...
  estimator = tf.estimator.Estimator(model_fn=LFNO.model_fn,
                                     model_dir=model_dir,
                                     config=run_config,
                                     params=params)
  ...
  early_stopping = tf.estimator.experimental.stop_if_no_increase_hook(
    ...
    max_steps_without_increase=2000,
    min_steps=100,
    run_every_secs=300) # TensorFlow 2
  ...
  serving_function = LFNO.json_serving_input_fn(feat_names=feat_col_names)
...
if __name__ == '__main__':
  ...
  tf.compat.v1.logging.set_verbosity(args.verbosity)  # TensorFlow 2
  # Set C++ Graph Execution level verbosity
  #os.environ['TF_CPP_MIN_LOG_LEVEL'] = str(tf.logging.__dict__[args.verbosity] / 10)
  os.environ['TF_CPP_MIN_LOG_LEVEL'] = str(tf.compat.v1.logging.__dict__[args.verbosity] / 10)
  config = args.__dict__
  for k, v in config.items():
    #tf.logging.info('{}: {}'.format(k, v))
    tf.compat.v1.logging.info('{}: {}'.format(k, v))  # TensorFlow 2
