...
DATA_DIR="/home/edmondium/power-consumption"
#DATA_DIR="./data"
...
then
  ...
  gcloud ai-platform local train $ALL_FLAGS
else
  ALL_FLAGS="--region $REGION "$ALL_FLAGS
  echo $ALL_FLAGS
  gcloud --project=${PROJECT_ID} ai-platform jobs submit training $JOB_NAME $ALL_FLAGS
fi
